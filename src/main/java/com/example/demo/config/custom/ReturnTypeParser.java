...
public interface ReturnTypeParser {
	...
	/**
	 * This is a copy of GenericTypeResolver.resolveType which is not available on spring
	 * 4. This also keeps compatibility with spring-boot 1 applications. Resolve the given
	 * generic type against the given context class, substituting type variables as far as
	 * possible.
			...
			if (genericType instanceof TypeVariable) {
				...
				if (resolvedType.hasUnresolvableGenerics()) {
					...
					return ResolvableType
							.forClassWithGenerics(Objects.requireNonNull(resolvedType.getRawClass()), resolvableTypes)
							.getType();
				}
			}
		}
	...
	static void resolveType(ResolvableType[] resolvableTypes, Class<?> contextClass) {
		for (int i = 0; i < resolvableTypes.length; i++) {
			if (resolvableTypes[i].hasUnresolvableGenerics()
					&& resolvableTypes[i].getType() instanceof ParameterizedType) {
				...
				if (resolvableTypes[i].getRawClass() != null) {
					resolvableTypes[i] = ResolvableType.forClassWithGenerics(
							Objects.requireNonNull(resolvableTypes[i].getRawClass()), resolvableTypes[i].getGenerics());
				}
			}
		}
	}
	...
	default Type getReturnType(MethodParameter methodParameter) {
		if (methodParameter.getGenericParameterType() instanceof ParameterizedType) {
			return ReturnTypeParser.resolveType(methodParameter.getGenericParameterType(),
